  ...
  <div class="main" >
      ...
      <mt-loadmore
        ...
        <div class="listcont" v-for="(item,index) in getCategoryArticleList" :key="index" >
            <router-link :to="{name:'readDtail',params:{corpusId:item.corpusId,curId:0,preId:0}}" class="l_top_box">
            ...
            <div class="l_box">
                ...
                <span class="l_left" v-if='item.readCount'>
                    <span class="l_num">{{item.readCount}}</span>
                    阅读
                </span>
                <span class="l_right">
                    <b>{{item.time}}</b>
                ...
                <div class="l_user" v-if="headhide">
                    <div class="userimg">
                        <a>
                    ...
                    <router-link :to="{path:'/readPersnal',query:{pernal_Id:-1}}" class="userbtn">他的首页</router-link>
                </div>
            </div>
        </div>
...
<script>
  ...
  export default {
    ...
    created(){
        ...
        let self = this;
        // console.log(this.$store.getCategoryArticleList)
    ...
    methods: {
        ...
        loadData(cb){
            //获取分类文章
            this.categoryId = this.$route.query.id === undefined ? -1 : this.$route.query.id;
